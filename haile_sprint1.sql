...
SELECT company_name AS Compañía, email AS Correo_Electronico, country AS País
FROM company
...
JOIN transaction
ON company.id = transaction.company_id
WHERE company_id in ("b-2354");

# Nivel 1 - Exercici 6: A més, el teu cap et sol·licita que indiquis quina és la companyia amb major despesa mitjana

SELECT company.company_name AS Compañía, AVG(transaction.amount) AS Transacciones_Medias
FROM company
JOIN transaction
ON company.id = transaction.company_id
GROUP BY company.company_name
ORDER BY Transacciones_Medias DESC LIMIT 1;
...
SELECT company.company_name AS Compañia, company.phone AS Telefono, company.country AS Pais, sum(transaction.amount) AS Importe
FROM company
JOIN transaction
ON company.id = transaction.company_id
